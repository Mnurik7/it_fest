// Панель специалиста: загрузка тикетов только своего отдела
...
async function loadTickets(showLoader = true) {
    ...
    params.append('specialist_view', 'true');
    ...
    try {
        ...
        if (data.success) {
            ...
            tbody.innerHTML = tickets.map(ticket => {
                const userName = ticket.user_id && usersMap[ticket.user_id] 
                    ? `${usersMap[ticket.user_id].username}` 
                    : 'Неизвестно';
                
                const sourceDisplay = ticket.source === 'phone' 
                    ? '<span style="color: #e74c3c; font-weight: bold;">📞 Звонок</span>' 
...
async function viewTicket(ticketId) {
    ...
    try {
        ...
        if (data.success) {
            ...
            const sourceLabel = ticket.source === 'phone' ? '📞 Звонок' : ticket.source;
...
async function loadUserInfo(userId) {
    ...
    try {
        ...
        if (response.status === 404) {
            // Пользователь не найден - это нормально, если тикет был создан без пользователя
...
function endRealtimeCall() {
    ...
    document.getElementById('aiAdviceRealtime').innerHTML = '<div style="color: #666; font-style: italic; text-align: center; padding: 2rem;">Начните вводить текст звонка, и ИИ начнёт давать советы...</div>';
...
async function analyzeRealtimeCall(text) {
    ...
    document.getElementById('aiAnalyzing').style.display = 'inline';
    ...
    try {
        ...
        if (data.success && data.analysis) {
            ...
            if (analysis.advice) {
                ...
                adviceHTML += `<strong style="color: #27ae60; display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.75rem; font-size: 1rem; font-weight: 600;">💡 Совет для оператора</strong>`;
            ...
            if (analysis.category) {
                ...
                if (analysis.department) {
                    adviceHTML += `<span style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 0.5rem 1rem; border-radius: 20px; font-size: 0.875rem; font-weight: 500; box-shadow: 0 2px 4px rgba(102, 126, 234, 0.3);"><strong>🏢</strong> ${escapeHtml(analysis.department)}</span>`;
                }
                adviceHTML += `</div>`;
            }
...
async function saveRealtimeCall() {
    ...
    // Показываем индикатор загрузки
    const saveBtn = document.querySelector('#saveCallBtnText');
...
async function handleCallSubmit(e) {
    ...
    // Показываем индикатор загрузки
    const callBtn = document.querySelector('#callForm .btn-success');
